...
def train(local_rank, args):
    ...
    dist.init_process_group(
        'nccl', rank=rank, world_size=world_size, timeout=datetime.timedelta(seconds=60)
    )

    torch.cuda.set_device(local_rank)
    model = MyNet()
    local_train_sampler = DDP_sampler(
        datasets.MNIST(
            './mnist_data',
            download=True,
            train=True,
...
def test(local_rank, args):
    ...
    local_test_loader = torch.utils.data.DataLoader(
        test_set, batch_size=128, shuffle=False, sampler=local_test_sampler
    )
    correct_total = 0
    with torch.no_grad():
...
def main():
    ...
    parser.add_argument(
        '-m', '--machines', default=2, type=int, help='number of machines'
    )
    parser.add_argument(
        '-g', '--gpus', default=4, type=int, help='number of GPUs in a machine'
    )
